...
export const cartSlice2 = createSlice({
  ...
  reducers: {
    ...
    removeItemFromCart: (
      ...
      if (itemIndex !== -1) {
        ...
        if (item.quantity > 1) {
          ...
          restaurantCart.items.slice(itemIndex, 1);
        }
      }
    ...
    updateCustomizableItem: (
      ...
      const targetConstimizationIndex = item?.customizations?.findIndex(
        cust => cust.id === customizationId,
      );
      if (targetConstimizationIndex === -1) return;

      const targetCustomization =
        item?.customizations[targetConstimizationIndex];
      ...
      if (matchingCustomizationIndex !== -1) {
        ...
        item?.customizations?.splice(targetConstimizationIndex, 1);
      } else {
...
export const selectCart = (state: RootState) => state.cart2;

export const selectRestaurantCart = (restaurantId: string) =>
  createSelector(
    (state: RootState) =>
      state.cart2.carts.find(cart => cart.restaurant.id === restaurantId),
