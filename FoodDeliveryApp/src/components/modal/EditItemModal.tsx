...
import { updateCustomizableItem } from '@states/reducers/cartSlice2';
...
const EditItemModal: FC<{ item: any; restaurant: any; onClose: () => void, cus: any }> = ({
  ...
  useEffect(() => {
    ...
    cus?.customizationOptions?.forEach((cusOption: any) => {
      const itemCustomization = item?.customizationOptions?.find((option: any) => option.type === cusOption?.type);

      if (itemCustomization) {
        const selectedIndex = itemCustomization?.options?.findIndex((option: any) => option?.name === cusOption?.selectedOption?.name);
        if (selectedIndex !== -1) {
